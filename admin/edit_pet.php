...
function updateBreeds(species, currentBreed = '') {
    ...
    breedData.forEach(breed => {
        if (!species || breed.species === species) {
    ...
    // If the current breed is not in the filtered list, clear the input
    // (Only if species is not empty to avoid clearing on initial load if species is valid)
    if (species && currentBreed && !breedData.some(b => b.name === currentBreed && b.species === species)) {
        // Only clear if the user is actively changing species
    }
}
...
document.getElementById('species-input').addEventListener('input', function() {
    ...
    const breedInput = document.getElementById('breed-input');
    breedInput.value = ''; // Clear breed input when species changes
    updateBreeds(species);
});

// Initial load
updateBreeds(document.getElementById('species-input').value, document.getElementById('breed-input').value);
